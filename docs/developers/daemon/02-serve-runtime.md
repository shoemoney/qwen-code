...
Before `runQwenServe()` starts this sequence, the CLI-only `--open --ephemeral-auth` mode validates loopback/Web Shell eligibility and fills `ServeOptions.token` with 32 random bytes encoded as base64url only when the existing option-over-environment selection is empty. Direct embedders and invocations without that default-off flag do not generate a token.
...
| CLI flags       | `--open --ephemeral-auth`                                                                                  | Default-off loopback Web Shell launch that reuses or generates a process-lifetime bearer before runtime.   |
