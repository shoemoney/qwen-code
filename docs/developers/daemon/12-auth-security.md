...
- The CLI-only `--open --ephemeral-auth` mode runs before boot: after deterministic loopback/Web Shell checks, it applies the same option-over-environment selection and fills `ServeOptions.token` with 32 random bytes encoded as base64url only when no non-empty selected token exists. The generated credential is process-lifetime, is not written to `process.env` or storage, and reaches the browser through the existing URL fragment. Bare `--open` and direct `runQwenServe()` callers never generate it.
...
| CLI flags       | `--open --ephemeral-auth`                                                               | Reuse or generate a loopback Web Shell bearer before daemon boot.       |
