...
The flag is default-off and requires `--open`, the Web Shell, built Web Shell assets, and a loopback bind. Bare `qwen serve --open` remains token-less on loopback. In ephemeral mode, normal API routes reject other local clients without the bearer; static Web Shell assets and loopback `/health` keep their existing pre-authentication behavior unless `--require-auth` is also set. For multiple clients or a reopenable Web Shell, use a stable shared token instead:
...
Daemon-backed `qwen channel set` and `qwen channel reload`, plus `status` and `stop` with `--daemon-url`, cannot discover a token generated by `--ephemeral-auth`. Use `QWEN_SERVER_TOKEN` and pass the same value with `--token` to those clients, or omit ephemeral mode.
...
`--open --ephemeral-auth` is a CLI-owned convenience, not another daemon token source: it selects `--token` when that option is defined (even if blank), otherwise `QWEN_SERVER_TOKEN`, then trims the selected value and generates only if the result is empty. The generated value lives only in the daemon process and the receiving browser tab. It is not stored, exported as an environment variable, shared with other tabs or clients, independently revocable, or tied to a client identity. Possession grants the same daemon authority as any other bearer token. See the [ephemeral authentication design](../design/2026-08-22-serve-open-ephemeral-auth.md) and related future work in [#4514](https://github.com/QwenLM/qwen-code/issues/4514).
