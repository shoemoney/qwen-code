...
Do not add `--ephemeral-auth` to this command. That mode delivers its generated bearer only to the tab opened by `--open`; the extension cannot discover it. To protect a daemon used by the extension, set `QWEN_SERVER_TOKEN` explicitly and configure the same stable token in every authorized client.
