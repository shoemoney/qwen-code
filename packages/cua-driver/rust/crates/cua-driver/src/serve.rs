//! Local daemon server and client for `qwen-cua-driver serve`/`stop`/`status`.
//!
...
/// worker (`qwen-cua-driver-uia.exe`). Public CLI, MCP, and SDK clients never connect
...
pub fn default_uia_pipe_path() -> String {
    if crate::bundle::is_local_installation() {
        r"\\.\pipe\qwen-cua-driver-local-uia".to_owned()
    } else {
        r"\\.\pipe\qwen-cua-driver-uia".to_owned()
    }
}
...
pub async fn run_serve(
    ...
    loop {
        tokio::select! {
            ...
            _ = &mut shutdown_rx => {
                eprintln!("Qwen Cua Driver daemon shutting down.");
                break;
            }
            _ = &mut parent_liveness => {
                eprintln!("Qwen Cua Driver embedded host closed its lifetime pipe; shutting down.");
                break;
            }
        }
    }
...
pub async fn run_serve(
    ...
    anyhow::bail!("qwen-cua-driver serve is not supported on this platform");
}

// ── CLI helpers ───────────────────────────────────────────────────────────────

/// `qwen-cua-driver serve` implementation.
pub fn run_serve_cmd(
    ...
    if is_daemon_listening(&socket_path) {
        let pid_hint = pid_file_path
            ...
            .unwrap_or_default();
        eprintln!(
            "Qwen Cua Driver daemon is already running on {socket_path}{pid_hint}. \
             Run `qwen-cua-driver stop` first."
        );
        std::process::exit(1);
    }
    ...
    #[cfg(target_os = "windows")]
    {
        if matches!(platform_windows::diagnostics::current_session_id(), Some(0),) {
            eprintln!(
                "WARNING: qwen-cua-driver serve is starting in Session 0 (services). \
    ...
    let sdk = match rt.block_on(crate::sdk_adapter::SdkAdapter::load(driver)) {
        Ok(sdk) => sdk,
        Err(error) => {
            eprintln!("qwen-cua-driver serve error: {error}");
            std::process::exit(1);
        }
    };
    if let Err(e) = rt.block_on(run_serve(sdk, &socket_path, pid_file_path.as_deref())) {
        eprintln!("qwen-cua-driver serve error: {e}");
        std::process::exit(1);
    }
}

/// `qwen-cua-driver stop` implementation.
        ...
        Ok(_) => {
            ...
            loop {
                ...
                if std::time::Instant::now() >= deadline {
                    eprintln!("Qwen Cua Driver daemon did not release socket within 2s");
                    std::process::exit(1);
                }
...
/// `qwen-cua-driver status` implementation.
pub fn run_status_cmd(socket_path: &str, pid_file_path: &str) {
    if is_daemon_listening(socket_path) {
        println!("Qwen Cua Driver daemon is running");
